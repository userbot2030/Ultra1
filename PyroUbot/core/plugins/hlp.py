...
async def menu_inline(client, inline_query):
    msg = f"<b>⌕ ʜᴇʟᴘ ᴍᴇɴᴜ sɪ ᴧꝛᴧʙ ᴜʙᴏᴛ</b>\n\nㅤ ⲇ ᴜꜱᴇʀ : <code><a href=tg://user?id={inline_query.from_user.id}>{inline_query.from_user.first_name} {inline_query.from_user.last_name or ''}</a></code>\nㅤㅤㅤㅤⲇ ᴘʀᴇꜰɪx : <code>{prefix}</code>"
...
async def menu_callback(client, callback_query):
    ...
    prefix = await ubot.get_prefix(client.me.id)
    top_text = f"<b>⌕ ᴍᴇɴᴜ ɪɴʟɪɴᴇ sɪ ᴧꝛᴧʙ ᴜʙᴏᴛ<b>\nㅤ ⲇ ᴜꜱᴇʀ : <code><a href=tg://user?id={callback_query.from_user.id}>{callback_query.from_user.first_name} {callback_query.from_user.last_name or ''}</a></code>"
    if mod_match:
