...
async def menu_inline(client, inline_query):
    msg = f"<b>⌕ ᴄᴍᴅ sᴧs ᴜʟᴛʀᴀ ᴜꜱᴇʀʙᴏᴛ\n\n⊗ ᴜꜱᴇʀ : <a href=tg://user?id={inline_query.from_user.id}>{inline_query.from_user.first_name} {inline_query.from_user.last_name or ''}</a></b>"
...
async def menu_callback(client, callback_query):
    ...
    prefix = await ubot.get_prefix(client.me.id)
    top_text = f"<b>⌕ ᴍᴇɴᴜ ɪɴʟɪɴᴇ <a href=tg://user?id={callback_query.from_user.id}>{callback_query.from_user.first_name} {callback_query.from_user.last_name or ''}</a></b>"
    if mod_match:
