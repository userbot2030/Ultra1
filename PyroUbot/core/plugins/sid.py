...
async def id_cmd(client, message):
    if len(message.command) < 2:
        ...
        if chat_type == ChatType.PRIVATE:
            ...
            _id = ""
            _id += f"<b>ɪᴅ {message.from_user.first_name} {message.from_user.last_name or ''} ᴀᴅᴀʟᴀʜ:</b> <code>{message.from_user.id}</code>\n<b>ID {message.chat.title} Adalah:</b> <code>{message.chat.id}</code>\n"
            if message.reply_to_message:
                ...
                if file_info:
                    _id += f"<b>ɪᴅ {file_info.message_type} ᴀᴅᴀʟᴀʜ:</b> <code>{file_info.file_id}</code>\n"
